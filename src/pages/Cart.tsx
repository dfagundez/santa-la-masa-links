...
import { MdArrowBack, MdRemoveShoppingCart, MdDelete, MdAdd, MdRemove } from 'react-icons/md';
...
const Cart: React.FC = () => {
  ...
  const handleSendOrder = () => {
    ...
    cartItems.forEach(item => {
      message += `▪️ ${item.quantity}x ${item.product.name} - $${formatPrice(item.product.price * item.quantity)}\n`;
    });
    ...
    window.open(`https://wa.me/+5491154793903?text=${encodedMessage}`, '_blank');
  };
          ...
          {cartItems.length === 0 ? (
            ...
            <div className="bg-white rounded-lg shadow-md p-6 border border-vainilla">
              ...
              <div className="divide-y divide-vainilla">
                {cartItems.map((item) => (
                  <div key={item.product.id} className="py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between">
                    <div className="flex items-center space-x-4 mb-3 sm:mb-0">
                      ...
                      />
                      <div>
                        <h3 className="text-md font-semibold text-budin font-playfair">{item.product.name}</h3>
                        <p className="text-sm text-gris">${formatPrice(item.product.price)} por unidad</p>
                      </div>
                    </div>
              ...
              <div className="mt-6 border-t border-vainilla pt-6">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-xl font-bold text-budin">${formatPrice(getTotal())}</span>
                </div>
                
                {/* Formulario para datos de envío */}
                <div className="mb-6 space-y-4">
                  <div>
                    <label htmlFor="name" className="block text-sm font-medium text-budin mb-1">Tu nombre</label>
                    <input
                      type="text"
                      id="name"
                      value={name}
                  ...
                  <div>
                    <label htmlFor="address" className="block text-sm font-medium text-budin mb-1">Dirección de entrega (opcional)</label>
                    <textarea
                      id="address"
                      rows={2}
                      value={address}
